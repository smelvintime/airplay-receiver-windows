using System;

namespace AirPlayReceiver.Protocol;

/// <summary>
/// Emulates Apple's FairPlay (<c>/fp-setup</c>) handshake using the well-known
/// fixed response tables — the "playfair" approach every open-source AirPlay
/// receiver uses. Real FairPlay DRM is not implemented; iOS only requires these
/// canned responses to proceed past fp-setup for a non-Apple receiver.
///
/// Two phases:
///   • setup  (16-byte request)  → 142-byte reply chosen by request[14] (mode).
///   • verify (164-byte request) → fp_header(12) + request[144..164] = 32 bytes,
///                                 and the request is retained as the "key
///                                 message" used later to decrypt the stream key.
///
/// Reference: RPiPlay lib/fairplay_playfair.c (response tables reproduced verbatim).
/// </summary>
public sealed class FairPlayHandshake
{
    /// <summary>The 164-byte phase-2 request, retained for stream-key decryption at SETUP.</summary>
    public byte[]? KeyMessage { get; private set; }

    /// <summary>
    /// Processes an fp-setup request body and returns the response body, or null
    /// if the request isn't a recognised fp-setup message.
    /// </summary>
    public byte[]? Process(byte[] request)
    {
        // Phase 1: 16-byte request, version byte at [4] must be 0x03.
        if (request.Length == 16)
        {
            if (request[4] != 0x03) return null;
            int mode = request[14];
            if (mode < 0 || mode >= ReplyMessages.Length) return null;
            KeyMessage = null;
            return (byte[])ReplyMessages[mode].Clone();
        }

        // Phase 2: 164-byte request → 32-byte reply, retain the request.
        if (request.Length == 164)
        {
            if (request[4] != 0x03) return null;
            KeyMessage = (byte[])request.Clone();
            byte[] response = new byte[32];
            Array.Copy(FpHeader, 0, response, 0, FpHeader.Length);
            Array.Copy(request, 144, response, 12, 20);
            return response;
        }

        return null;
    }

    private static readonly byte[] FpHeader =
    {
        0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x14,
    };

    private static readonly byte[][] ReplyMessages =
    {
        new byte[]
        {
            ,
        },
        new byte[]
        {
            ,
        },
        new byte[]
        {
            ,
        },
        new byte[]
        {
            ,
        },
    };
}
